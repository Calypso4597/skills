...
author: simoncorry
source: https://github.com/simoncorry/foundry
homepage: https://github.com/simoncorry/foundry/blob/main/.agents/skills/test-it/SKILL.md
---
...
Ground rules: run it in the background with a stated time limit in its own prompt; verify the inspection surface responds before spawning (a grader pointed at a dead environment burns its budget on a vacuous non-verdict); one grader per invocation, no loops. And spawn it only after the state it judges is committed and pushed: a grader reads the tree as it stands at spawn time, so work that lands minutes later reads as missing. When skew is unavoidable, name it in the grader's prompt and adjudicate the report against it rather than dismissing the finding as a false alarm. Every failure it produces is a Step 3 red case. If your tool can't spawn subagents, skip in one line and say so.
